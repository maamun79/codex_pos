...
function mi_db_tbl_like($tbl_name, $condition, $column, $attribute_1, $attribute_2 = NULL, $order_by = NULL, $order_type = NULL, $limit = NULL){

    if ($condition == 'ANY'){
        if ($order_by != NULL && $limit != NULL){
            $sql = "SELECT * FROM `".$tbl_name."` WHERE `".$column."` LIKE '%".$attribute_1."%' ORDER BY `".$order_by."` ".$order_type." LIMIT ".$limit;
            echo $sql;
        }elseif ($order_by != NULL){
            if ($order_type == NULL){
                $order_type = 'ASC';
            }
            $sql = "SELECT * FROM `".$tbl_name."` WHERE `".$column."` LIKE '%".$attribute_1."%' ORDER BY `".$order_by."` ".$order_type;
        ...
        if ($attribute_2 != NULL){
            if ($order_by != NULL && $limit != NULL){
                $sql = "SELECT * FROM `".$tbl_name."` WHERE `".$column."` LIKE '".$attribute_1."%".$attribute_2."' ORDER BY `".$order_by."` ".$order_type." LIMIT ".$limit;
            }elseif ($order_by != NULL){
                if ($order_type == NULL){
                    $order_type = 'ASC';
                }
                $sql = "SELECT * FROM `".$tbl_name."` WHERE `".$column."` LIKE '".$attribute_1."%".$attribute_2."' ORDER BY `".$order_by."` ".$order_type;
